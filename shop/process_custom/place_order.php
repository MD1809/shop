...
<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_button'])){
        if (isset($_POST['items']) && is_array($_POST['items'])) {
            foreach ($_POST['items'] as $product_id => $itemData) {
                $product_id = (int)$product_id;
                $quantity = max(1, (int)$itemData['quantity']);
                
                if (isset($_SESSION['order_items'][$product_id])) {
                    $_SESSION['order_items'][$product_id]['quantity'] = $quantity;
                }
            }
        }
        ...
        if (isset($_POST['items'])) {
            foreach ($_POST['items'] as $product_id => $itemPost) {
                $product_id = (int)$product_id;
                $newQty = max(1, (int)$itemPost['quantity']); // tránh số 0 hoặc âm

                // Cập nhật trong SESSION order_items (ưu tiên)
                if (isset($_SESSION['order_items'][$product_id])) {
                    $_SESSION['order_items'][$product_id]['quantity'] = $newQty;
                }

                // Cập nhật trong cart (phòng trường hợp giỏ hàng cũng chứa sản phẩm này)
